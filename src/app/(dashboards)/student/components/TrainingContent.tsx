...
export function TrainingContent({ dashboardData, isLoading }: { dashboardData: any; isLoading: boolean }) {
  const trainingModules = [
    { name: 'Pre-Flight Procedures', progress: 100, status: 'completed', lessons: 8 },
    { name: 'Basic Flight Maneuvers', progress: 90, status: 'in-progress', lessons: 12 },
    { name: 'Navigation & Cross Country', progress: 75, status: 'in-progress', lessons: 10 },
    { name: 'Emergency Procedures', progress: 60, status: 'in-progress', lessons: 6 },
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                      ...
                      module.status === 'pending' ? 'bg-yellow-100 text-yellow-600' :
                      'bg-gray-100 text-gray-400'
                    }`}>
                      {module.status === 'completed' ? <CheckCircle className="w-4 h-4" /> :
                       module.status === 'locked' ? <AlertCircle className="w-4 h-4" /> :
                  ...
                  <Badge variant={module.status === 'completed' ? 'default' : 'outline'} 
                         className={module.status === 'completed' ? 'bg-green-600' : 'border-[#f6d57f] text-[#f6d57f]'}>
                    {module.status === 'completed' ? 'Completed' :
                     module.status === 'in-progress' ? 'In Progress' :
                     module.status === 'pending' ? 'Pending' : 'Locked'}
                  </Badge>
                </div>
                ...
                <div key={index} className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-sm text-[#f6d57f]">{skill.score}%</span>
                    </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {[
                ...
                { title: 'Cross Country Navigation', date: '2024-02-10', icon: MapPin },
                { title: 'Night Flying Endorsement', date: '2024-02-05', icon: Clock },
